...
export const en = {
  ...
  'how.body':
    ...
    '<p><b>What leaves with the file.</b> Both exports name themselves. The CSV opens with comment lines carrying the tool, its version, its DOI and the query that produced the rows, so a table that has been mailed on can still be cited and re-run; the JSON has always carried the same. The signature can be switched off for a reader that does not skip comments. Both files also carry the other names an account answers to, as <code>credit_name</code> and <code>other_names</code>: the display name is built from the given and family fields, and those are often a legal or transliterated form that no publication uses. The CSV also splits that list into one column per name, <code>other_name_1</code> onward, because a spreadsheet cannot sort or filter inside a cell. Those columns sit after every other one and appear only when someone in the result declares a variant.</p>',
