export const es = {
  ...
  'rail.family': 'Parte de <a href="https://rijdho.github.io/metaudits-home/" target="_blank" rel="noreferrer">Metadata Audits</a>',
  ...
  'how.body':
    ...
    '<p><b>Qué sale con el archivo.</b> Las dos exportaciones se identifican. El CSV empieza con líneas de comentario que nombran la herramienta, su versión, su DOI y la consulta que produjo las filas, de modo que una tabla reenviada sigue siendo citable y repetible; el JSON lo lleva desde siempre. La firma puede desactivarse si el archivo va a un lector que no salta los comentarios. Ambos archivos llevan además los otros nombres que constan en la cuenta, como <code>credit_name</code> y <code>other_names</code>: el nombre mostrado se arma con los campos de nombre y apellido, y esos son a menudo una forma legal o transliterada con la que nadie publica. El CSV además reparte esa lista en una columna por nombre, <code>other_name_1</code> en adelante, porque una hoja de cálculo no puede ordenar ni filtrar dentro de una celda. Esas columnas van después de todas las demás y solo aparecen si alguien del resultado declaró una variante.</p>',
